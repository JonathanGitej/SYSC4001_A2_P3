...
#include "interrupts_101294584_101302762.hpp"
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            execution += std::to_string(current_time) + ", " + std::to_string(fork_duration) + ", cloning PCB\n";
            ...
            if (!allocate_memory(&child)) {
                std::cerr << "ERROR DURING FORK: memory allocation failed for child process.\n";
                continue;
            }
            ...
            system_status += "time: " + std::to_string(current_time) + "; current trace: FORK, " + std::to_string(fork_duration) + "\n";
            ...
            auto [child_exec, child_sys, child_time] = simulate_trace(child_trace, current_time, vectors, delays, external_files, child, wait_queue);
            ...
            if (program_size == 0) { //size of 0 means the program was most likely not found
                execution += "ERROR DURING EXEC: Program " + program_name + " not found!\n";
                continue;
            }
            ...
            //free memory and allocate program
            free_memory(&current);

            //6 because the test templates had them like that
            ...
            if (!allocate_memory(&current)) {
                std::cerr << "ERROR DURING EXEC: memory allocation failed.\n" << std::endl;
                continue;
            }

            unsigned int loading_time = program_size * 15; //15 time units per MB
            ...
            //3 because the test templates had them like that
            ...
            auto [exec_exec, exec_sys, exec_time] = simulate_trace(exec_traces, current_time, vectors, delays, external_files, current, wait_queue);

            execution += exec_exec;
            system_status += exec_sys;
